...
class BornReportController extends Controller
{
    protected $Data = [];
    protected $Module = 'Penduduk';
    protected $Title = 'LAPORAN KELAHIRAN DESA SELANGNANGKA';
    ...
    public function Anything($request)
    {
        ...
        $Data = Born::select(
            ...
            )->join("family", "born.family_id", "family.id")
            ->join("users", "users.id", "family.head_id")
            ->whereBetween('born.created_at', [$From, $End])->get(); 

        if (isset($Data)) {
            foreach ($Data as $Value) {
                $sheet->setCellValue('A'.$this->StartRowValue, $Number++);
                $sheet->setCellValue('B'.$this->StartRowValue, $Value->name);
                $sheet->setCellValue('C'.$this->StartRowValue, Carbon::parse($Value->date_of_birth)->format('Y-m-d'));
                $sheet->setCellValue('D'.$this->StartRowValue, ($Value->gender == 'l') ? "Laki-Laki" : "Wanita");
                $sheet->setCellValue('E'.$this->StartRowValue, $Value->number_family);
                $sheet->setCellValue('F'.$this->StartRowValue, $Value->head_name);
        ...
        $FileName = 'report/kelahiran-' . Carbon::now() . '.xlsx';
